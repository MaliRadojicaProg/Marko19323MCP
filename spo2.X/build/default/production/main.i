...
# 157 "main.c"
    ADCON0 = ADCON0 & (0xC3);
    ADCON0 = ADCON0 + ((BR_KANALA & 0x0F) << 2);
...
void akvizicija() {
    ...
    ByteX[0] = 0xAA;
    ByteX[11] = (unsigned char)(Red >> 2);
    ByteX[12] = (unsigned char)(Infrared >> 2);
    ByteX[13] = (unsigned char)(((Red & 0x0003) << 6) + ((Infrared & 0x0003) << 2));


    if (NRed > 922) ByteX[4] += 12;
    else if (NRed > 204) ByteX[4] += 4;
    else ByteX[4] += 0;

    if (NInfrared > 922) ByteX[4] += 3;
    else if (NInfrared > 204) ByteX[4] += 1;
    else ByteX[4] += 0;
