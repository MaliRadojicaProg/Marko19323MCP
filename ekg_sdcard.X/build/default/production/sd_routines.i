...
unsigned char SD_sendCommand(unsigned char cmd, unsigned long arg) {
    ...
    while ((response = SPI1_Read(0xFF)) == 0xFF)
        if (retry++ > 0xfe) break;

    if ((response == 0x00) && (cmd == 58))
    {
        status1 = (SPI1_Read(0xFF) & 0x40);
        if (status1 == 0x40) SDHC_flag = 1;
        else SDHC_flag = 0;
...
unsigned char SD_erase(unsigned long startBlock, unsigned long totalBlocks) {
    ...
    response = SD_sendCommand(32, startBlock);

    if (response != 0x00)
        return response;

    response = SD_sendCommand(33, (startBlock + totalBlocks - 1));

    if (response != 0x00)
        return response;

    response = SD_sendCommand(38, 0);
...
unsigned char SD_readSingleBlock(unsigned long startBlock, unsigned char * Buffer1) {
    ...
    response = SD_sendCommand(17, startBlock);
...
unsigned char SD_writeSingleBlock(unsigned long startBlock, unsigned char * Buffer1) {
    ...
    response = SD_sendCommand(24, startBlock);
...
unsigned char SD_readMultipleBlock(unsigned long startBlock, unsigned char * Buffer1, unsigned char * Buffer2) {
    ...
    response = SD_sendCommand(18, startBlock);
...
unsigned char SD_writeMultipleBlock(unsigned long startBlock, unsigned char upis_Buffer1,
    ...
    totalBlocks = 2;
    blockCounter = 0;
    response = SD_sendCommand(25, startBlock);
    if (response != 0x00) return response;
